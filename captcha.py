...
def captcha(gt: str, challenge: str, endpoint: str = "http://127.0.0.1:9645/pass_uni"):
    """
    通用的验证码处理函数
    参数:
        gt: 验证码的 gt 参数
        challenge: 验证码的 challenge 参数
        endpoint: API 的端点 URL，默认为 "http://127.0.0.1:9645/pass_uni"
    返回:
    ...
    try:
        res = httpx.get(
            endpoint,
            # 移除已废弃的 use_v3_model 参数，仅保留核心入参
